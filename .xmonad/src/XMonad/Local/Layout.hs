...
import           XMonad.Layout.Decoration      as D
...
layoutHook =
 ...
 where
  ...
  full    = noBorders Full
  tall =
    avoidStruts $ windowNavigation $ addTabs' $ addSubLayout' $ spacing $ mkCols
      0
      1
      2
      (3 / 5)
      (1 / 100)
      RightToLeft
  grid  = avoidStruts $ windowNavigation $ addTabs' $ addSubLayout' $ spacing Grid
  mcols = avoidStruts $ windowNavigation $ addTabs' $ addSubLayout' $ spacing $ mkCols
    1
    1
    3
    (3 / 8)
  ...
  addTabs' l = addTabs D.shrinkText tabConfig l
...
--------------------------------------------------------------------------------
-- | Themes for tabs and topbar & Functions
tabConfig :: D.Theme
tabConfig = def { fontName = "-*-terminus-medium-*-*-*-*-160-*-*-*-*-*-*"
                , activeColor         = "#424242"
                , inactiveColor       = "#009ec1"
                , activeBorderColor   = "#424242"
                , inactiveBorderColor = "#009ec1"
                , activeTextColor     = "#3F3F3F"
