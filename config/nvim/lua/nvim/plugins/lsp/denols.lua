...
local function buf_cache(bufnr)
    ...
    lsp.buf_request(bufnr, 'deno/cache', params,
                    function(err) if err then error(tostring(err)) end end)
end
...
configs[server_name] = {
    default_config = {
        ...
        init_options = {enable = true, lint = false, unstable = true},
        handlers = {
    ...
    commands = {
        ...
        DenolsReferences = {
            function() denols_references {includeDeclaration = true} end,
            description = 'List references. This handle deno:/ schema in deno:// buffer.'
        },
        DenolsCache = {
            function() buf_cache(0) end,
