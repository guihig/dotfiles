...
import XMonad.Layout.SubLayouts (GroupMsg(MergeAll, UnMerge), pullGroup, toSubl)
...
windowKeys _
 ...
 =
  ...
  [("M-C-" ++ k, windowSwap d False) | (k, d) <- zip arrowKeys directions2D]
...
screenKeys _ =
  ...
  [("M-S-" ++ k, windowToScreen d False) | (k, d) <- zip arrowKeys directions2D]
