...
import XMonad.Util.NamedScratchpad (customFloating, namedScratchpadManageHook)
...
manageHook =
  composeOne
    ...
    [ (className =? "Chromium-browser" <&&> stringProperty "WM_WINDOW_ROLE" =?
       "pop-up") -?>
      normalTile
  ...
  manageDocks <+> fullscreenManageHook <+> manageSpawn
  where
...
floatDynamicPropEventHook =
  dynamicPropertyChange "WM_CLASS" dynamicClassComposed <+>
  ...
  where
    ...
    dynamicNameComposed =
      composeAll [title =? "Android Emulator*" --> forceCenterFloat]
